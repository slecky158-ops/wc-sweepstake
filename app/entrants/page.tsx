      ...
      <div className="px-5 sm:px-8 pt-5">
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {sorted.map((e, i) => {
            ...
            return (
              <div key={e.slug} className="surface p-3 sm:p-4 min-w-0 hover:border-gold transition-colors">
                <Link href={`/entrants/${e.slug}`} className="flex items-baseline justify-between gap-2 mb-2.5 min-w-0 group">
                  <div className="display text-[15px] sm:text-lg text-text truncate min-w-0 group-hover:text-gold transition-colors">{e.name}</div>
                  <span className="num text-[10px] text-text-faint shrink-0">{String(i + 1).padStart(2, '0')}</span>
                </Link>
                <div className="space-y-1.5 text-[13px] sm:text-sm">
                  {a && (
                    ...
                    >
                      ...
                      <span className="text-text truncate font-semibold min-w-0">{a.name}</span>
                  ...
                  {b && (
                    ...
                    >
                      ...
                      <span className="text-text truncate font-semibold min-w-0">{b.name}</span>
